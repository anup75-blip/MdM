"""
bill_class5.py
Class 5 Government Bill — mirrors 'Bill 5th' sheet of the government Excel.
"""
...
st.set_page_config(page_title="Bill — Class 5", layout="wide", initial_sidebar_state="expanded")

user = require_login()

with st.sidebar:
    st.title("Bill — Class 5")
...
st.title("Bill — इ.5 (Class 5)")
render_bill_header(user, bill_label="इ.5 (Class 5)", month_label=month_label)

if month_df.empty or month_df["class5"].sum() == 0:
    st.info("No Class 5 attendance entered for this month yet.")
    st.stop()

filled_df = month_df[month_df["total"] > 0]
c5_bill, _ = calculate_bill(
...
m2.metric("Student-Days",   c5_bill.total_student_days)
m3.metric("Total Bill",     f"₹{c5_bill.total_expense:,.2f}")
...
exp_rows.append({
    ...
    "Rate (₹/day)": "",
    "Student-Days": "",
    "Amount (₹)":   f"**₹{c5_bill.total_expense:,.2f}**",
})
...
# ── Food consumed ─────────────────────────────────────────────────────────────
st.subheader("Food Consumed — Class 5")
food_rows = []
for i, fl in enumerate(c5_bill.food_lines, start=1):
    food_rows.append({
        "Sr.":          i,
